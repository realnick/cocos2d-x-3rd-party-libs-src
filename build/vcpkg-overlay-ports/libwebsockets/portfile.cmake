...
# UWP-specific source fixes.
# gettimeofday.c has mixed CRLF/LF endings and needs UWP guards for the
# _tzset()/_timezone/_daylight block plus a <winsock2.h> include for
# `struct timeval`. Simplest / most robust: overwrite the small file.
# windows-file.c calls desktop-only CreateFileW / GetFileSize that are
# missing from the UWP lib, so we substitute CreateFile2 / GetFileSizeEx.
if(VCPKG_CMAKE_SYSTEM_NAME STREQUAL "WindowsStore")
    file(WRITE "${SOURCE_PATH}/win32port/win32helpers/gettimeofday.c" "
#include <time.h>
#include <winsock2.h>
#include <windows.h>
...
")
    ...
    # if_nametoindex is exported from WindowsApp.lib for every UWP
    # architecture, but netioapi.h only declares it for the desktop partition,
    # so network.c calls it with no declaration in scope and C assumes __cdecl.
    # On x64/arm64 that still binds, because those have no name decoration. On
    # x86 the __cdecl reference `_if_nametoindex` cannot bind to the __stdcall
    # import `__imp__if_nametoindex@4`, and the DLL fails to link with LNK2019.
    # Declaring it with the right calling convention fixes x86 and is a no-op
    # elsewhere.
    vcpkg_replace_string(
        "${SOURCE_PATH}/lib/core-net/network.c"
        "#include \"private-lib-core.h\"\n#include <errno.h>"
        "#include \"private-lib-core.h\"\n#include <errno.h>\n\nEXTERN_C DECLSPEC_IMPORT NET_IFINDEX WINAPI if_nametoindex(PCSTR InterfaceName);"
    )

    # Suppress C4013 (implicit function declaration warning) for UWP.
    # Other Winsock/iphlpapi APIs are in the same situation as if_nametoindex
    # above -- present in the UWP-linked libs, declaration hidden behind
    # WINAPI_PARTITION_DESKTOP. Suppressing this warning avoids the
    # /WX-induced error while still letting the linker resolve the symbols.
    # Note this is only safe where the calling convention happens to match;
    # any further x86-uwp LNK2019 on a hidden API wants a declaration like the
    # one above, not another warning suppression.
    vcpkg_replace_string(
        "${SOURCE_PATH}/CMakeLists.txt"
        "add_compile_options(/W3 /WX /wd4142 /wd4267 /wd4996)"
        "add_compile_options(/W3 /WX /wd4013 /wd4142 /wd4267 /wd4996)"
    )

    file(READ "${SOURCE_PATH}/lib/plat/windows/windows-file.c" _wf_src)
    string(REPLACE
        "	if (((*flags) & 7) == _O_RDONLY)\n		ret = CreateFileW(buf, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,\n				  NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);\n	else\n		ret = CreateFileW(buf, GENERIC_WRITE, 0, NULL,\n				  CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);"
        "	{\n		CREATEFILE2_EXTENDED_PARAMETERS xp = {0};\n		xp.dwSize = sizeof(xp);\n		xp.dwFileAttributes = FILE_ATTRIBUTE_NORMAL;\n		if (((*flags) & 7) == _O_RDONLY)\n			ret = CreateFile2(buf, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_EXISTING, &xp);\n		else\n			ret = CreateFile2(buf, GENERIC_WRITE, 0, CREATE_ALWAYS, &xp);\n	}"
        _wf_src "${_wf_src}")
    string(REPLACE
        "	fop_fd->len = GetFileSize(ret, NULL);"
        "	fop_fd->len = 0;"
        _wf_src "${_wf_src}")
    file(WRITE "${SOURCE_PATH}/lib/plat/windows/windows-file.c" "${_wf_src}")
endif()
...
# LWS_WITH_LIBUV is off everywhere on purpose.
#
# It only adds libuv as an optional event-loop backend, selected at runtime with
# LWS_SERVER_OPTION_LIBUV. cocos2d-x never asks for it -- neither v3 nor v4
# references LWS_SERVER_OPTION_LIBUV or any lws_uv_* API -- so lws runs its
# default poll loop and the libuv code is dead weight.
#
# Leaving it on costs both cocos2d-x trees. The static build v3 links pulls in
# 21 undefined uv_* symbols, and v3 has no external\uv directory at all to
# resolve them from; the shared build v4 uses makes websockets.dll import
# uv.dll, which then has to be deployed alongside it. Turning it off removes
# the dependency on both sides.
set(EXTRA_ARGS "-DLWS_WITH_LIBUV=OFF")
